...
// #express-router
// initialized express router here
// then made my routes for the different requests
...
router.get("/", (req, res) => {
  Reviews.find()
    ...
    .catch((err) =>
      res
        .status(500)
        .json({ message: "Failed to get reviews from database", error: err })
    );
});
...
router.put("/:id", restricted, checkReviewId, checkIfAuthorized, (req, res) => {
  ...
  Reviews.update(id, body)
    ...
    .catch((err) =>
      res
        .status(500)
        .json({ errorMessage: "Failed to update review", error: err })
    );
});
  ...
  (req, res) => {
    Reviews.remove(req.params.id)
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .json({ errorMessage: "Failed to delete review", error: err });
      });
  }
);
